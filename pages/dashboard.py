...
try:
    ...
    st.error(f"scraper.py import failed: {e}")
    st.error(traceback.format_exc())
    module_status["scraper"] = False

try:
    from analysis import analyze_sentiment, compute_kpis, extract_keywords
    st.write("analysis.py: functions imported successfully")
    module_status["analysis"] = True
except Exception as e:
    st.error(f"analysis.py import failed: {e}")
    st.error(traceback.format_exc())
...
try:
    ...
    st.error(f"report_gen.py import failed: {e}")
    st.error(traceback.format_exc())
...
try:
    ...
    st.warning(f"servicenow_integration.py import failed: {e}")
...
competitors_input = st.text_area(
    "Competitors (separate multiple brands with commas)", 
    value="Competitor1, Competitor2"
)
competitors = [c.strip() for c in competitors_input.split(",") if c.strip()]

campaign_messages_input = st.text_area(
    "Campaign Messages for MPI (separate with commas)", 
...
if st.button("Analyze"):
    if not module_status.get("scraper") or not module_status.get("analysis"):
        ...
        try:
            ...
            sentiments = [item.get('sentiment','neutral') for item in data.get('full_data',[])]
            
            # Sentiment analysis (even if some missing)
            _, tones = analyze_sentiment(mentions)
            ...
            if module_status.get("servicenow_integration") and (neg_pct > 50 or any('nytimes.com' in m.get('source','') and m.get('sentiment','neutral')=='negative' for m in data.get('full_data',[]))):
                try:
...
if st.session_state['kpis']:
    ...
    with col1:
        ...
        sentiment_dict = kpis.get('sentiment_ratio', {})
        sentiment_df = pd.DataFrame({
            'Tone': list(sentiment_dict.keys()), 
            'Percentage': list(sentiment_dict.values())
        })
        if not sentiment_df.empty:
            fig_pie = px.pie(sentiment_df, values='Percentage', names='Tone', title='Sentiment Ratio')
            st.plotly_chart(fig_pie)

        # SOV chart (safe padding/truncating)
        all_brands = [brand] + competitors
        sov_values = kpis.get('sov', [])
        if len(sov_values) < len(all_brands):
            sov_values += [0]*(len(all_brands)-len(sov_values))
        elif len(sov_values) > len(all_brands):
            sov_values = sov_values[:len(all_brands)]
...
if st.button("Generate PDF Report"):
    if not module_status.get("report_gen"):
